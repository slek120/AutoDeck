...
def ValuePerCost(card):
    # Value / Cost
    return getValue(card)/getCost(card)
...
for i in range(maxCards):
    ...
    if( sum([getCost(card) for card in deck]) + getCost(collection[-1]) <= maxCost ): # -1 = last in list
...
while( len(collection) > 0 ):
    ...
    for i in range(len(deck)):
        ...
        if( sum([getCost(card) for card in deck]) - getCost(deck[i]) + getCost(collection[-1]) <= maxCost ):
